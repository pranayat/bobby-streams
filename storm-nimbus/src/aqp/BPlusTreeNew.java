...
public class BPlusTreeNew<K extends Comparable<? super K>, V extends Tuple> {
	...
	public BPlusTreeNew(int branchingFactor) {
	 ...
	 */
	public void delete(K key, String id) {
		root.deleteValue(key, id);
	}
		...
		while (!queue.isEmpty()) {
			...
			while (!queue.isEmpty()) {
				...
				while (it.hasNext()) {
					...
					if (it.hasNext())
						sb.append(", ");
					if (node instanceof BPlusTreeNew.InternalNode)
	...
	private abstract class Node {
		...
		abstract void deleteValue(K key, String id);
		...
		public void sortKeysAndValues(List<K> keys, List<V> values) {
        ...
        // Sort indices based on the values from array1
        Collections.sort(indices, Comparator.comparing(keys::get));

        // Create sorted lists using the sorted indices
		...
		public void sortKeysAndChildren(List<K> keys, List<Node> children) {
			// Create an array of indices
			List<Integer> indices = new ArrayList<>();
			for (int i = 0; i < keys.size(); i++) {
					indices.add(i);
			...
			for (Integer index : indices) {
				sortedKeys.add(keys.get(index));
				sortedChildren.add(children.get(index));
			}
		...
		public int findIndex(List<K> keys, K key) {
			for (int i = 0; i < keys.size(); i++) {
					// found
	...
	private class InternalNode extends Node {
		...
		@Override
		void deleteValue(K key, String id) {
			Node child = getChild(key);
			child.deleteValue(key, id);
		...
		void insertChild(K key, Node child) {
			...
			// } else {
				...
				sortKeysAndChildren(keys, children);
			// }
		}
	...
	private class LeafNode extends Node {
		...
		@Override
		void deleteValue(K key, String id) {
			int loc = findIndex(keys, key, values, id);
			if (loc >= 0) {
				keys.remove(loc);
				values.remove(loc);
			}
