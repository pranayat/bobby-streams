...
public class JoinQueryNew {
    ...
    public JoinQueryNew(String id, double radius, List<String> streamIds, List<String> fields, Distance distance, IDistance iDistance) {
    ...
    public List<Tuple> execute(Tuple tuple, QueryGroup queryGroup) {
        ...
        int j = 0;
        while (j < unjoinedStreams.size()) { // for [a1, b1] in [ [a1, b1], [a1, b2] ]

          int levelSize = joinCombinations.size();

          for (int i = 0; i < levelSize; i++) {

            String streamToJoin = unjoinedStreams.get(j);
            List<Tuple> joinCombination = joinCombinations.poll();
